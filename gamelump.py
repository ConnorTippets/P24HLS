...
class GameLumps:
    ...
    def set_lump_from_id(
        self, id: bytes, gamelump: GameLump, gamelump_header: GameLumpHeader
    ):
        lump_index = 0
...
class GameLumpConverter:
    def __init__(
        self,
        gamelump_offset: int,
        new_gamelump_offset: int,
        new_prps_version: int,
        reader: Reader,
        writer: Writer,
    ):
    ...
    def convert(self):
        ...
        _, staticgamelump_flags, staticgamelump_version, _, _ = astuple(
            gamelump.find_header_from_id(STATICPROPSGAMELUMP_ID)
        )
        ...
        with io.BytesIO(staticgamelump_data) as handle_in:
            with io.BytesIO() as handle_out:
                ...
                StaticPropsGameLumpConverter(
                    reader, writer, staticgamelump_version, self.new_prps_version
                ).convert()

                staticgamelump_out = handle_out.getvalue()

        gamelump.set_lump_from_id(
            STATICPROPSGAMELUMP_ID,
            GameLump(staticgamelump_out),
            GameLumpHeader(
                STATICPROPSGAMELUMP_ID,
                staticgamelump_flags,
                self.new_prps_version,
                0,
                0,
            ),
        )
